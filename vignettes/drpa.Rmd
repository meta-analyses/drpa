...
dtable <- readr::read_csv('../inst/extdata/default_cutoff.csv')
...
| censor_method        | This determines whether any censor method applies to the dose-response relationship. There are four possibilities: `none`, `default`(to use the default cut-off, determined by the meta-analysis), `WHO-DRL` (WHO's recommended physical activity per week `8.75` when doubled, so `17.5` MMETs hours per week) or `WHO-QRL` (WHO's recommended physical activity per week `8.75` when quadrupled, so `35` MMETs hours per week) . The default value is `default` - a fixed censor point at determined by the underlying meta-analysis after which the relationship remains the same. |
