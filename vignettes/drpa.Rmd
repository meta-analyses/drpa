...
The first column `disease` specifies the supported diseases in the package. The other three columns, which are outcome types (`fatal`, `non-fatal` and `fatal-and-non-fatal`), show the last knot (based on the available data points) for the specific disease. We don't have confidence that our meta-analysis is reliable after this point, and assumes a flat relationship after this point. Whenever a disease such as `all-cause-cvd` doesn't have an outcome type `non-fatal`, it will be `NA`. More will follow in the next section.
...
drpa::dose_response(cause = 'all-cause-cvd', outcome_type = "fatal-and-non-fatal", dose = 40, confidence_intervals = TRUE, certainty = TRUE)
...
| outcome_type         | Fatal, non-fatal or fatal-and-non-fatal                                                               |
